...
export class AuthService {
  ...
  signup(name: string, mobile_number: string, email: string): Observable<{ mobile_number: string; message: string; dev_otp: string | null }> {
    return this.http.post<{ mobile_number: string; message: string; dev_otp: string | null }>(
      `${API_BASE}/signup`, { name, mobile_number, email }, OPTS
    );
  }

  sendOtp(mobile_number: string, purpose: OtpPurpose): Observable<{ message: string; dev_otp: string | null }> {
    return this.http.post<{ message: string; dev_otp: string | null }>(
      `${API_BASE}/otp/send`, { mobile_number, purpose }, OPTS
    );
  }

  verifyOtp(mobile_number: string, otp: string, purpose: OtpPurpose): Observable<{ otp_verified_token: string }> {
    return this.http.post<{ otp_verified_token: string }>(
      `${API_BASE}/otp/verify`, { mobile_number, otp, purpose }, OPTS
    );
  }

  setPin(otp_verified_token: string, pin: string): Observable<{ user: UserProfile }> {
    return this.http.post<{ user: UserProfile }>(`${API_BASE}/pin/set`, { otp_verified_token, pin }, OPTS);
  }

  resetPin(otp_verified_token: string, pin: string): Observable<{ user: UserProfile }> {
    return this.http.post<{ user: UserProfile }>(`${API_BASE}/pin/reset`, { otp_verified_token, pin }, OPTS);
  }

  pinLogin(identifier: string, pin: string): Observable<{ user: UserProfile }> {
    return this.http.post<{ user: UserProfile }>(`${API_BASE}/pin/login`, { identifier, pin }, OPTS);
  }

  completeMobile(otp_verified_token: string): Observable<{ user: UserProfile }> {
    return this.http.post<{ user: UserProfile }>(`${API_BASE}/complete-mobile`, { otp_verified_token }, OPTS);
  }
